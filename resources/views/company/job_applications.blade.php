    ...
    @if($users->count()>0)
        ...
        @foreach($users as $user)
            <div class="application mb-4">
                ...
                <span class="status
                @if($user->pivot->status  == 'Applied') {{'bg-primary'}}
                @elseif($user->pivot->status  == 'Viewed') {{'bg-secondary'}}
                @elseif($user->pivot->status  == 'Selected') {{'bg-info'}}
                @elseif($user->pivot->status  == 'Not selected') {{'bg-danger'}}
                @else($user->pivot->status  == 'In consideration') {{'bg-success'}} @endif
                    font-weight-bolder pr-1 pl-1 d-block mb-4">{{$user->pivot->status}}
                </span>

                <div class="font-weight-bolder mt-4">
                    <a href="" data-user-id="{{$user->id}}" data-job-id="{{$job->id}}"
                       class="viewd text-decoration-none user-name"><span
                            class="pr-3 dblock">{{$user->first_name . $user->last_name}}</span></a>
                    <span class="d-block">{{$user->email}}</span>
                </div>
                <hr>
                <a class="pr-3 btn btn-primary change-status" data-toggle="modal"
                   data-target="#changestatus{{$user->id}}">Change status</a>
                <div class="modal fade" id="changestatus{{$user->id}}" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="{{route('company.job.user.status.update',[$user->id , $job->id])}}"
                                      method="post">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        ...
                                        <select name="status" class="form-control" id="status">
                                            <option
                                                value="applied" @if($user->pivot->status == "Applied") {{"selected"}}  @endif>
                                                Applied
                                            </option>
                                            <option
                                                value="viewed" @if($user->pivot->status == "Viewed") {{"selected"}}  @endif>
                                                Viewed
                                            </option>
                                            <option
                                                value="selected" @if($user->pivot->status == "selected") {{"selected"}}  @endif>
                                                Selected
                                            </option>
                                            <option
                                                value="In consideration" @if($user->pivot->status == "In consideration") {{"selected"}}  @endif>
                                                In consideration
                                            </option>
                                            <option
                                                value="Not selected" @if($user->pivot->status == "Not selected") {{"selected"}}  @endif>
                                                Not selected
                                            </option>
                                        </select>
                                    </div>
