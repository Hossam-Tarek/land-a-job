...
<body class="bg-white">
<div class="container-fluid row m-0 justify-content-center align-items-center" id="interval"
     style="background-image: url({{asset('img/guest/category/category13.jpg')}});">
    <nav class="navbar navbar-expand-lg text-white fixed-top ">
        ...
        <div class="collapse custom-navbar-collapse navbar-collapse my-5 " id="navbarSupportedContent">
            ...
            <form class="form-inline my-2 my-lg-0">
                <a href="{{route('login')}}" class="btn btn-secondary  mr-sm-2 text-white nav-font mx-1">Log in</a>
                <a href="{{route('register')}}" class="btn btn-secondary  mr-sm-2 text-white nav-font mx-1">Register</a>
                <a href="{{route('company-register')}}" class="btn btn-success my-2 my-sm-0" type="submit"><i
                        class="fa fa-user mx-1"></i>Employer?</a>
            </form>
        </div>
    </nav>
    {{-- Search A Job --}}
    <div class="container search-container">
        @if( Session::has( 'success' ))
            <div class="alert alert-success p-2 position-fixed w-100"
                 style="top: 80px; z-index: 1000000"> {{ Session::get( 'success' ) }} </div>
        @endif
        <div class="form-group search row ">
            <div class="col-lg-12 mb-3 ">
                <h2 class="mx-auto text-white font-weight-bolder d-flex justify-content-center">Find the Best Jobs
                    in
                    Egypt</h2>
                <h5 class="mx-auto text-white  font-weight-bolder my-3 d-flex justify-content-center">Searching for
                    vacancies & career opportunities? WUZZUF helps you in your job search in Egypt
                </h5>
            </div>
            <div class="col-lg-12">
                <form action="{{route('job.search.index')}}" id="job-search-form" method='get'>
                    <div class="d-flex">
                        <div class="text-center job-search-container px-0 col-12 offset-0 col-lg-8 offset-lg-2">
                            <input class="form-control w-100 p-4 m-0 d-inline-block mr-5 job-search-input-guest"
                                   type="text" name="q" id="job-search-keyword"
                                   placeholder="Search jobs (e.g. Work from Home)">
                            <button class="btn btn-primary search-button" id="job-search-button"
                                    type='submit'>Search Jobs
                            </button>
                        </div>
                    </div>
                </form>
        ...
        <div class="row">
                ...
                <div class="col-lg-3 col-sm-6 khaled">
                    <div class="card latest-job my-2">
                        <div class="card-body box-shad">
                            <h5 class="card-title"><a href="{{route('user.show-job',$job->id)}}">{{$job->title}}</a>
                            </h5>
                            <span class="card-text">{{$job->company->name}}</span>
                            <span> - {{$job->city->name}}</span>
                            <p class="card-text"><small
                                    class="text-muted">{{\Illuminate\Support\Carbon::now()->diffForHumans(\Illuminate\Support\Carbon::parse($job->created_at))}}
                                    Ago</small></p>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-sm-12">
                <div class="float-right my-3">
                    <a href="{{route('job.search.index')}}">See all new Jobs on Land A Job</a>
                </div>
            </div>
        </div>
        <hr class="my-4 mx-5">
            ...
            @foreach ($careerlevels as $index => $careerlevel)
                @if($index < 6)
                    <div class="col-lg-4 col-sm-12 my-3">
                        <div class="box-shad">
                            <a href="{{route('job.search.index')}}">
                                <img class="career-level" src="{{asset('img/guest/career/'.$career[$index])}}"
                                     alt="Career Level">
                            </a>
                ...
                @foreach ($industries as $index => $industry)
                    @if($index < 3)
                        <div class="col-lg-4 col-sm-12 my-4">
                            <div class="box-shad">
                                <a href="{{route('job.search.index')}}">
                                    <img class="career-level"
                                         src="{{asset('img/guest/category/'.$category[$index])}}"
                                         alt="Industry Category">
                                </a>
                                <h5 class="position-absolute text-light text-bottom p-2">{{$industry->name}}
                                    Jobs</h5>
                            </div>
                        </div>
                    @else
                        <div class="col-lg-4  collapse show" id="category-jobs">
                            <h5 class=" text-bottom p-2"><a href="{{route('job.search.index')}}">{{$industry->name}}
                                    Jobs</a></h5>
                        </div>
                    @endif
                @endforeach
                <div class="col-sm-12">
                    <button class=" btn btn-primary  w-100 d-lg-none justify-content-center d-flex" type="button"
                            data-toggle="collapse" data-target="#category-jobs" aria-controls="category-jobs"
    ...
    @if ($cities->count() > 0)
        <div class="jobs-by-category my-5">
            ...
            <div class="row ">
                @foreach ($cities as $index => $cit)
                    @if($index < 3)
                        <div class="col-lg-4 col-sm-12 my-4">
                            <div class="box-shad">
                                <a href="{{route('job.search.index')}}">
                                    <img class="career-level" src="{{asset('img/guest/city/'.$city[$index])}}"
                                         alt="City">
                                </a>
                    ...
                    @else
                        <div class="col-lg-4 collapse show" id="city-jobs">
                            <h5 class=" text-bottom p-2"><a href="{{route('job.search.index')}}">Jobs
                                    in {{$cit->name}}</a></h5>
                        </div>
                    @endif
                @endforeach
                <div class="col-sm-12">
                    <button class=" btn btn-primary  w-100 d-lg-none justify-content-center d-flex" type="button"
                            data-toggle="collapse" data-target="#city-jobs" aria-controls="city-jobs"
                            aria-expanded="true" aria-label="Toggle navigation">
                        <span class="navbar-toggler-icon"></span>
                        show more
                    </button>
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-4 col-sm-12  mb-md-0 p-5 text-white text-left">
                ...
                <a href="#" class="font-awoesm text-muted" id="website-facebook"><i
                        class="fab fa-facebook-f pr-3"></i></a>
                <a href="#" class="font-awoesm text-muted" id="website-twiter"><i
                        class="fab fa-twitter pr-3"></i></a>
                <a href="#" class="font-awoesm text-muted" id="website-linkedin"><i
                        class="fab fa-linkedin-in pr-3"></i></a>
                <a href="#" class="font-awoesm text-muted" id="website-whatsapp"><i
                        class="fab fa-whatsapp pr-3"></i></a>
            </div>
            ...
            <div class="col-lg-4  mb-4 mb-md-0  p-5 text-left" id="contactus">
                <form action="{{route('admin.messages.store')}}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input class="footer-input form-control" name="email" type="email" required>
                    </div>
                    @error('email')
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="footer-input form-control" name="message_content"></textarea>
                    </div>
